...
BINARY_FILE = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'target.bin')

# Memory map for the code to be tested
CODE_ADDRESS  = 0x800000b4  # BBB image has entry point at 0x80000000. Assuming this is correct?
CODE_SIZE_MAX = 0x00010000  # Max size for the code (64kb)
STACK_ADDRESS = 0xa0000000  # Address of the stack (arbitrarily chosen). Chose something larger than entry point.
STACK_SIZE	  = 0x00010000  # Size of the stack (arbitrarily chosen)
DATA_ADDRESS  = 0x80030000  # Address where mutated data will be placed
DATA_SIZE_MAX = 0x00010000  # Maximum allowable size of mutated data

#IO peripherals
UART_START = 0x48022000
UART_END = 0x48022FFF
CTRL_MOD_START = 0x44E10000
CTL_MOD_END = 0x44E11FFF
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Test harness for BBB FreeRTOS app image.")
    parser.add_argument('input_file', type=str, help="Path to the file containing the mutated input to load")
    parser.add_argument('-d', '--debug', default=False, action="store_true", help="Enables debug tracing")
    args = parser.parse_args()

    # Instantiate a ARM32 little endian Unicorn Engine instance
    ...
    uc.mem_map(CODE_ADDRESS, 2*1024*1024)
    uc.mem_write(CODE_ADDRESS, binary_code)

    # Set the program counter to the start of the code
    start_address = CODE_ADDRESS          # Address of entry point of main()
    end_address   = CODE_ADDRESS + 0xf8   # Address of last instruction in main()
    uc.reg_write(UC_ARM_REG_PC, start_address)

    #-----------------
    # Setup the stack

    uc.mem_map(STACK_ADDRESS, STACK_SIZE)
    uc.reg_write(UC_ARM_REG_SP, STACK_ADDRESS + STACK_SIZE)
    ...
    try:
        uc.emu_start(uc.reg_read(UC_ARM_REG_PC), 0, 0, count=1)
    except UcError as e:
    ...
    # Write the mutated command into the data buffer
    uc.mem_map(DATA_ADDRESS, 10*1024*1024)
    ...
    try:
        result = uc.emu_start(uc.reg_read(UC_ARM_REG_PC), end_address, timeout=0, count=0)
    except UcError as e:
