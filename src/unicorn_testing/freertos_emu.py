...
def hook_code(uc, address, size, user_data):
    ...
    for i in md.disasm(tmp, address):
        ...
        reg = mu.reg_read(UC_ARM_REG_R0)
        reg1 = mu.reg_read(UC_ARM_REG_R1)  
        reg2 = mu.reg_read(UC_ARM_REG_R2)  
        reg3 = mu.reg_read(UC_ARM_REG_R3)  
        pc = mu.reg_read(UC_ARM_REG_PC)  
        sl = mu.reg_read(UC_ARM_REG_SL)

        print('>>> R0 = 0x%x; R1 = 0x%x; R2 = 0x%x; R3 = 0x%x; PC = 0x%x; SL = 0x%x' %(reg, reg1, reg2, reg3, pc, sl) )  
        #print('>>> R0 = 0x%x; R1 = 0x%x; PC = 0x%x' %(reg, reg1, pc) )  

def mapmemory(mu):
	filename = "memmap.json"
	total_size = 0
	with open(filename) as fp:
		data = json.load(fp)
		print("Opened memmap.json. Mapping addresses into Unicorn...")
		for function in data['function']:
			address = function['base']
			size = int(function['size'])
			print("Allocating %d words at %s for function %s" % (size, address, function['name']))	
			total_size += size	
	if(total_size*4 < 4096):		
		print("Less than 1 page needed... mapping a single page into unicorn")
		#mu.mem_map(ADDRESS, 4096)
		return total_size*4
	else:
		total_size = int(4096 * round(float(total_size*4)/4096))
		#mu.mem_map(ADDRESS, total_size)
		print("Mapping %d bytes into unicorn" %(total_size))
		return total_size
...
try:
        ...
        instr_mem = mapmemory(mu)
        mu.mem_map(ADDRESS, 512*1024*1024)

        #Open code from file
        file = open("target.bin.bak","rb")
        CODE = file.read(instr_mem)
        ...
        mu.emu_start(ADDRESS, ADDRESS + 0xf8)
